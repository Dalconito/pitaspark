        ...
        <div class="adesaodiv col-10 col-s-10">
            <div class="adesaotexto">
                ...
                <div class="col-s-12 col-adesao">
                    ...
                    <a href="https://wa.me/+5511983506830?text=Vim pela Landing Page, gostaria de mais informações" class="Consultarbtn" id="btnSaibaMais">Clique aqui e Saiba Mais!</a>
                </div>
            </div>
        </div>
    </section>
...
<?php 
$requestPost = isset($_SERVER['REQUEST_METHOD'])? $_SERVER['REQUEST_METHOD'] : null;
if ($requestPost === 'POST') {
    // Recebe os dados enviados pelo fetch
    $data = json_decode(file_get_contents('php://input'), true);
    
    if (isset($data['click']) && $data['click'] === true) {
        $file = 'clicks.log';
        $current = file_get_contents($file);
        $current .= "Clique registrado em: " . date("Y-m-d H:i:s") . "\n";
        $current .= "Click e Data" . $logEntry;
        $current .= "------------------- \n";
